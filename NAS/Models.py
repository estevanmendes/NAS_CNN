...
def create_model(individual,pool_of_features,pool_of_features_probability,debug=False)-> tf.keras.Sequential:
    """
    ...
    for (index,gene) in enumerate(individual):
        ...
        if layer_details['layer'] is not None:
            ...
            if len(model.layers)==0:
                ...
                layer=layer_details['layer'](**layer_details['params'])     
                #deprecated due feasiable_model function in ga. 
    ...
    if len(model.layers)==0:
        layer=tf.keras.layers.Dense(1,activation='sigmoid',input_shape=(100,100,3))
    else:
